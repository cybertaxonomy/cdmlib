...
public class EntityValidationCrudJdbcImpl implements IEntityValidationCrud {
	...
	@Override
	public <T extends ICdmBase> void saveEntityValidation(T validatedEntity, Set<ConstraintViolation<T>> errors, CRUDEventType crudEventType,
			Class<?>[] validationGroups)
	{
		...
		try {
			...
			if (previousResult == null) {
				/*
				 * The entity has never been validated before. We should now create an
				 * entityvalidation record whether or not the entity has errors, because
				 * the entity HAS been validated so its validationcount is now 1.
				 */
				entityValidationId = saveValidationResultRecord(conn, validatedEntity, crudEventType);
				if (errors.size() != 0) {
					saveErrorRecords(conn, entityValidationId, validatedEntity, errors);
				}
			}
			else {
				entityValidationId = previousResult.getId();
				deletedErrorRecords(conn, entityValidationId, validationGroups);
				if (errors.size() != 0) {
	...
	private static void deletedErrorRecords(Connection conn, int validationResultId, Class<?>[] validationGroups) throws SQLException
	{
	...
	private static <T extends ICdmBase> int saveValidationResultRecord(Connection conn, T entity, CRUDEventType crudEventType) throws SQLException
	{
		PreparedStatement stmt = null;
	...
	private static void updateValidationResultRecord(Connection conn, int validationResultId, CRUDEventType crudEventType) throws SQLException
	{
		...
		try {
			...
			stmt.setInt(4, validationResultId);
			stmt.executeUpdate();
		}
		finally {
			JdbcDaoUtils.close(stmt);
	...
	private static <T extends ICdmBase> void saveErrorRecords(Connection conn, int validationResultId, T entity, Set<ConstraintViolation<T>> errors)
			throws SQLException
	{
		PreparedStatement stmt = null;
		try {
	...
	private static <T extends ICdmBase> EntityValidation getEntityValidationRecord(Connection conn, String validatedEntityClass, int validatedEntityId)
			throws SQLException
	{
	...
	private static int getValidationResultId(Connection conn, String validatedEntityClass, int validatedEntityId) throws SQLException
	{
	...
	private static Set<EntityConstraintViolation> getErrorRecords(Connection conn, int validationResultId) throws SQLException
	{
