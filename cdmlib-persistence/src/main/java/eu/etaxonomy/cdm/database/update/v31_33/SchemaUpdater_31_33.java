...
// ********************** FACTORY METHOD *******************************************
	...
	protected List<ISchemaUpdaterStep> getUpdaterList() {
		...
		step = SimpleSchemaUpdaterStep.NewNonAuditedInstance(stepName, query).setDefaultAuditing("Representation");
		stepList.add(step);
		...
		step = SimpleSchemaUpdaterStep.NewNonAuditedInstance(stepName, updateSql).setDefaultAuditing("DefinedTermBase");
		stepList.add(step);
...
//		step = SimpleSchemaUpdaterStep.NewNonAuditedInstance(stepName, query).setAuditing("Reference");
//		stepList.add(step);
...
//		stepList.add(step);
	...
	private void updateIdInVocabulary(List<ISchemaUpdaterStep> stepList) {
		...
		ISchemaUpdaterStep step = SimpleSchemaUpdaterStep.NewNonAuditedInstance(stepName, query).setDefaultAuditing(tableName); //not fully correct as we should join with TermVoc_AUD but good enough for this usecase 
		stepList.add(step);
		...
		step = SimpleSchemaUpdaterStep.NewNonAuditedInstance(stepName, query).setDefaultAuditing("Representation");  //AUD not needed
		stepList.add(step);
	...
	private void updateTermTypeForVocabularies(List<ISchemaUpdaterStep> stepList, TermType termType) {
		...
		ISchemaUpdaterStep step = SimpleSchemaUpdaterStep.NewNonAuditedInstance(stepName, query).setDefaultAuditing("TermVocabulary");  //AUD not fully correct as subselect should also work on AUD, good enough for our purposes
		stepList.add(step);
	}
	...
	private void updateTermTypesForTerms(List<ISchemaUpdaterStep> stepList) {
		...
		String query = " UPDATE DefinedTermBase " + 
				...
				" WHERE DTYPE = '" + NamedArea.class.getSimpleName() + "' OR DTYPE = 'TdwgArea' OR DTYPE = 'WaterbodyOrCountry' OR DTYPE = '"+ Country.class.getSimpleName() + "' OR DTYPE = 'Continent' ";
		ISchemaUpdaterStep step = SimpleSchemaUpdaterStep.NewNonAuditedInstance(stepName, query).setDefaultAuditing(tableName);
		stepList.add(step);
	...
	private void updateElevationMax(List<ISchemaUpdaterStep> stepList) {
		...
		step = SimpleSchemaUpdaterStep.NewNonAuditedInstance(stepName, query).setDefaultAuditing(audTableName);
		stepList.add(step);
		//TODO same for AUD
	...
	private void updateOriginalSourceType(List<ISchemaUpdaterStep> stepList) {
		...
		String query = String.format("UPDATE OriginalSourceBase " +
				" SET %s = '%s' ", typeAttrName, OriginalSourceType.Unknown.getKey());
		step = SimpleSchemaUpdaterStep.NewNonAuditedInstance(stepName, query).setDefaultAuditing(tableName);
		stepList.add(step);
		...
		query = String.format("UPDATE OriginalSourceBase " +
				" SET %s = '%s' " +
				" WHERE " +
