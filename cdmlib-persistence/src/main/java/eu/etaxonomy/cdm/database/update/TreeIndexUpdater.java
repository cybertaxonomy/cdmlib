 ...
 *
 */
public class TreeIndexUpdater extends SchemaUpdaterStepBase<TreeIndexUpdater> implements ISchemaUpdaterStep {
	...
	public Integer invoke(ICdmDataSource datasource, IProgressMonitor monitor) throws SQLException {
		boolean result = true;
		result &= indexTable(tableName, datasource, monitor);
		if (includeAudTable){
			String aud = "_AUD";
			result &= indexTable(tableName + aud, datasource, monitor);
		}
		return (result == true )? 0 : null;
	}

	private boolean indexTable(String tableName, ICdmDataSource datasource, IProgressMonitor monitor) throws SQLException {
		boolean result = true;			
