...
public class WfoContentExportResultProcessor {
    ...
    public WfoContentExportResultProcessor(WfoContentExportState state) {
        super();
        this.state = state;
        Map<String,String[]> resultMap;
        for (WfoContentExportTable table: WfoContentExportTable.values()){
    ...
    public void put(WfoContentExportTable table, String id, String[] csvLine) {
    ...
    public boolean hasRecord(WfoContentExportTable table, String id){
        Map<String, String[]> resultMap = result.get(table);
        if (resultMap == null){
            return false;
        }else{
    ...
    public  String[] getRecord(WfoContentExportTable table, String id){
        return result.get(table).get(id);
    }

    public void put(WfoContentExportTable table, ICdmBase cdmBase, String[] csvLine) {
       this.put(table, cdmBase.getUuid().toString(), csvLine);
    }

    public void createFinalResult(WfoContentExportState state) {
        ...
        if (!result.isEmpty() ){
            ...
            for (WfoContentExportTable table: result.keySet()){
    ...
    private String createCsvLine(WfoContentExportConfigurator config, String[] csvLine) {
        String lineString = "";
        boolean first = true;
