...
public class Abcd206XMLFieldGetter {
	...
	protected void getType(Node result) {
		...
		try {
			...
			logger.warn("NULLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLL");
				...
				for (int m = 0; m < identifications.getLength(); m++) {
					if (identifications.item(m).getNodeName()
						...
						for (int k = 0; k < results.getLength(); k++) {
							...
							if (results.item(k).getNodeName()	.equals(prefix + "TaxonIdentified")) {tmpName = this.getScientificName(results		.item(k));// logger.info("TMP NAME " + tmpName);dataHolder.identificationList.add(tmpName);nameFound = true;
							}
			...
			if (taxonsIdentified.item(l).getNodeName()
				...
				for (int n = 0; n < scnames.getLength(); n++) {
					...
					if (scnames.item(n).getNodeName()
						...
						try {
							if (scnames.item(n).hasChildNodes()) {String tmp = scnames.item(n).getChildNodes()		.item(1).getNodeName();if (tmp.indexOf(prefix) != -1		&& prefix.length() > 0)	dataHolder.nomenclatureCode = tmp	.split(prefix)[1];else {	dataHolder.nomenclatureCode = scnames			.item(n).getChildNodes().item(1)			.getNodeName();}
							}
						} catch (Exception e) {
				...
				for (int j = 0; j < multimedias.getLength(); j++) {
					if (multimedias.item(j).getNodeName()
						...
						for (int k = 0; k < multimedia.getLength(); k++) {
							if (multimedia.item(k).getNodeName()	.equals(prefix + "FileURI")) {dataHolder.multimediaObjects.add(multimedia		.item(k).getTextContent());path = multimedia.item(k).getNodeName();getHierarchie(multimedia.item(k));dataHolder.knownABCDelements.add(path);path = "";
							}
						}
					}
				}
				...
				for (int j = 0; j < childs.getLength(); j++) {
					if (childs.item(j).getNodeName().equals(prefix + "Person")) {
						...
						for (int k = 0; k < person.getLength(); k++) {
							if (person.item(k).getNodeName()	.equals(prefix + "FullName")) {path = person.item(k).getNodeName();getHierarchie(person.item(k));dataHolder.knownABCDelements.add(path);path = "";dataHolder.gatheringAgentList.add(person		.item(k).getTextContent());
							}
						}
					}
