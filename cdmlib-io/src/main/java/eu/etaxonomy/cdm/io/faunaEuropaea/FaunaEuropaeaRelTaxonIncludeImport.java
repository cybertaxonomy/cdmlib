...
//		taxonStore.makeEmpty();
	//	taxonStore = null;
		...
		//gets the taxon "Animalia" 
		
		//List<TaxonBase> taxon = getTaxonService().findByTitle(Taxon.class, "Animalia%", MatchMode.BEGINNING, null, null, 0, null, null).getRecords();
	...
	private boolean processParentsChildrenByID(FaunaEuropaeaImportState state) {
		...
		String selectColumns = " SELECT Taxon.TAX_ID AS ChildId, Parent.TAX_ID AS ParentId ";
		...
		try {
	        ...
	        while (rs.next()) {
				...
				if ((i++ % limit) == 0) {
					...
					txStatus = startTransaction();
					childParentMap = new HashMap<Integer, Integer>(limit);
				...
				Integer childIdInt = rs.getInt("ChildId");
				Integer parentIdInt = rs.getInt("ParentId");
				//UUID childUuid = UUID.fromString(childUuidStr);
				//UUID parentUuid = UUID.fromString(parentUuidStr);
				
				if (!childParentMap.containsKey(childIdInt)) {

						childParentMap.put(childIdInt, parentIdInt);
						if(logger.isInfoEnabled()) {
							logger.info("child ID added " + childIdInt + " parent ID " + parentIdInt); 
						}
						
				} else {
					if(logger.isInfoEnabled()) {
						logger.info("Duplicated child ID (" + childIdInt + ")"); 
					}
					
					if(logger.isDebugEnabled()) {
						logger.debug("Duplicated child ID (" + childIdInt + ")");
					}
				}
				if (((i % limit) == 0 && i != 1 ) || i == count) { 

					success = createParentChildRelationshipsById(state, childParentMap);
	...
	private boolean createParentChildRelationshipsById(
			FaunaEuropaeaImportState state, Map<Integer, Integer> childParentMap) {
		boolean success = true;
		int limit = state.getConfig().getLimitSave();
			...
			Set<Integer> childKeysSet = childParentMap.keySet();
			Set<Integer> parentValuesSet = new HashSet<Integer>(childParentMap.values());
			
			if (logger.isTraceEnabled()) {
				logger.trace("Start reading children and parents");
			}
			List<TaxonBase> children = getTaxonService().findTaxaByID(childKeysSet);
			logger.info("number of children " + children.size());
			List<TaxonBase> parents = getTaxonService().findTaxaByID(parentValuesSet);
			logger.info("number of parents " + parents.size());
			Map<Integer, TaxonBase> parentsMap = new HashMap<Integer, TaxonBase>(parents.size());
			for (TaxonBase taxonBase : parents){
				parentsMap.put(taxonBase.getId(), taxonBase);
			}
			
			if (logger.isTraceEnabled()) {
				logger.debug("End reading children and parents");
				for (Integer id : childKeysSet) {
					logger.trace("child id query: " + id);
				}
				for (Integer id : parentValuesSet) {
					logger.trace("parent id query: " + id);
				}
				for (TaxonBase tb : children) {
					logger.trace("child uuid result: " + tb.getUuid() + "child id: " + tb.getId());
				}
				for (TaxonBase tb : parents) {
					logger.trace("parent uuid result: " + tb.getUuid() + "parent id: " + tb.getId());
				}
			}

			Integer mappedParentUuid = null;
			Integer childUuid = null;

			for (TaxonBase child : children) {

				try {
					if (child instanceof Taxon){
					logger.error(child.getTitleCache() + "is a taxon!");
						Taxon childTaxon = child.deproxy(child, Taxon.class);
					childUuid = childTaxon.getId();
...
//					}
					...
					if (childTaxon != null && parentTaxon != null) {
						...
						logger.error(child.getName() + "is no taxon");
					}
	...
	private boolean processMisappliedNamesByID(FaunaEuropaeaImportState state) {
		...
		String selectColumns = " SELECT Taxon.TAX_ID AS MisappliedId, Parent.TAX_ID AS AcceptedId ";
		...
		try {
			...
			while (rs.next()) {
				...
				Integer childIDInt = rs.getInt("MisappliedId");
				Integer parentIDInt = rs.getInt("AcceptedId");
				//UUID childUuid = UUID.fromString(childUuidStr);
				//UUID parentUuid = UUID.fromString(parentUuidStr);
				...
				if (((i % limit) == 0 && i != 1 ) || i == count) { 
					...
					success = createMisappliedNameRelationshipsByID(state, childParentMap);
	...
	private boolean createMisappliedNameRelationshipsByID(
			FaunaEuropaeaImportState state, Map<Integer, Integer> fromToMap) {
		...
		boolean success = true;
			...
			Set<Integer> misappliedNamesSet = fromToMap.keySet();
			Set<Integer> acceptedTaxaSet = new HashSet<Integer>(fromToMap.values());
			
			if (logger.isTraceEnabled()) {
				logger.trace("Start reading misapplied names and accepted taxa");
			}
			List<TaxonBase> misappliedNames = getTaxonService().findTaxaByID(misappliedNamesSet);
			List<TaxonBase> acceptedTaxa = getTaxonService().findTaxaByID(acceptedTaxaSet);
			...
			if (logger.isTraceEnabled()) {
				...
				for (Integer id : misappliedNamesSet) {
					logger.trace("misapplied name id query: " + id);
				}
				for (Integer id : acceptedTaxaSet) {
					logger.trace("accepted taxon id query: " + id);
				}
				for (TaxonBase tb : misappliedNames) {
					logger.trace("misapplied name uuid result: " + tb.getUuid() + "misapplied name id result: " + tb.getId());
				}
				for (TaxonBase tb : acceptedTaxa) {
					logger.trace("accepted taxon uuid result: " + tb.getUuid()+ "accepted taxon id result: " + tb.getId());
				}
			}

			Integer mappedAcceptedTaxonId = null;
			Integer misappliedNameId = null;
			...
			for (TaxonBase misappliedName : misappliedNames) {
				...
				try {
					...
					misappliedNameId = misappliedNameTaxon.getId();
					mappedAcceptedTaxonId = fromToMap.get(misappliedNameId);
					acceptedTaxonBase = null;
					
					acceptedTaxonBase = acceptedTaxaMap.get(mappedAcceptedTaxonId);
							if (logger.isDebugEnabled()) {
								logger.debug("Parent (" + mappedAcceptedTaxonId + ") found for child (" + misappliedNameId + ")");
							}
					...
					if (misappliedNameTaxon != null && acceptedTaxon != null) {
						...
						if (logger.isDebugEnabled()) {
							logger.debug("Accepted taxon / misapplied name (" + mappedAcceptedTaxonId + "-" + misappliedNameId + 
							") relationship created");
						}
						if (!misappliedNameSet.contains(misappliedNameTaxon)) {
							...
							if (logger.isTraceEnabled()) {
								logger.trace("Misapplied name taxon (" + misappliedNameId + ") added to Set");
							}
							
						} else {
							if (logger.isDebugEnabled()) {
								logger.debug("Duplicated misapplied name taxon (" + misappliedNameId + ")");
							}
						}
					} else {
						if (logger.isDebugEnabled()) {
							logger.debug("Accepted taxon (" + mappedAcceptedTaxonId + ") or misapplied name (" + misappliedNameId + " is null");
						}
					}
					...
					logger.error("Error creating misapplied name relationship accepted taxon-misapplied name (" + 
						mappedAcceptedTaxonId + "-" + misappliedNameId + ")", e);
				}
	...
	private boolean processHeterotypicSynonymsByID(FaunaEuropaeaImportState state, String fromClause) {
		...
		String selectColumns = " SELECT Taxon.TAX_ID AS SynonymUuid, Parent.TAX_ID AS AcceptedUuid ";
	...
	private boolean storeSynonymRelationshipsByID(ResultSet rs, int count, FaunaEuropaeaImportState state) 
	throws SQLException {

		TransactionStatus txStatus = null;
		Map<Integer, Integer> synonymAcceptedMap = null;
		int i = 0;
		boolean success = true;
		int limit = state.getConfig().getLimitSave();
		...
		while (rs.next()) {
			...
			if ((i++ % limit) == 0) {
				...
				txStatus = startTransaction();
				synonymAcceptedMap = new HashMap<Integer, Integer>(limit);
			...
			Integer synonymIdInteger = rs.getInt("SynonymUuid");
			Integer acceptedIdInteger= rs.getInt("AcceptedUuid");
			//UUID synonymUuid = UUID.fromString(synonymUuidStr);
			//UUID acceptedUuid = UUID.fromString(acceptedUuidStr);

			if (!synonymAcceptedMap.containsKey(synonymIdInteger)) {

				synonymAcceptedMap.put(synonymIdInteger, acceptedIdInteger);

			} else {
				if(logger.isDebugEnabled()) {
					logger.debug("Duplicated synonym UUID (" + synonymIdInteger + ")");
				}
			}

			if (((i % limit) == 0 && i != 1 ) || i == count) { 

				success = createHeterotypicSynonymsByID(state, synonymAcceptedMap);
	...
	private boolean createHeterotypicSynonymsByID(
			FaunaEuropaeaImportState state,
			Map<Integer, Integer> fromToMap) {
		int limit = state.getConfig().getLimitSave();
		boolean success = true;

		Set<TaxonBase> synonymSet = new HashSet<TaxonBase>(limit);

		Set<Integer> synonymIdSet = fromToMap.keySet();
		Set<Integer> acceptedTaxaIdSet = new HashSet<Integer>(fromToMap.values());

		if (logger.isTraceEnabled()) {
			logger.trace("Reading synonym names and accepted taxa...");
		}
		List<TaxonBase> synonyms = getTaxonService().findTaxaByID(synonymIdSet);
		List<TaxonBase> acceptedTaxa = getTaxonService().findTaxaByID(acceptedTaxaIdSet);
		Map<Integer, TaxonBase> acceptedTaxaMap = new HashMap<Integer, TaxonBase>(acceptedTaxa.size());
		for (TaxonBase taxonBase : acceptedTaxa){
			acceptedTaxaMap.put(taxonBase.getId(), taxonBase);
		}

		if (logger.isTraceEnabled()) {
			logger.trace("End reading synonyms names and accepted taxa");
			for (Integer id : synonymIdSet) {
				logger.trace("synonym uuid query: " + id);
			}
			for (Integer id: acceptedTaxaIdSet) {
				logger.trace("accepted taxon uuid query: " + id);
			}
		...
		for (TaxonBase synonymTaxonBase : synonyms) {
			...
			try {
				...
				mappedAcceptedTaxonId = fromToMap.get(synonymId);
				acceptedTaxonBase = null;

				acceptedTaxonBase = acceptedTaxaMap.get(mappedAcceptedTaxonId);
				if (logger.isDebugEnabled()) {
					logger.debug("Parent (" + mappedAcceptedTaxonId + ") found for child (" + synonymId + ")");
				}
				acceptedTaxon = acceptedTaxonBase.deproxy(acceptedTaxonBase, Taxon.class);
				...
				if (synonym != null && acceptedTaxon != null) {
					...
					if (logger.isDebugEnabled()) {
						logger.debug("Accepted taxon - synonym (" + mappedAcceptedTaxonId + " - " + synonymId + 
						") relationship created");
					}
					if (synonym != null && !synonymSet.contains(synonym)) {
						...
						if (logger.isTraceEnabled()) {
							logger.trace("Synonym (" + synonymId + ") added to Set");
						}

					} else {
						if (logger.isDebugEnabled()) {
							logger.debug("Duplicated synonym (" + synonymId + ")");
						}
					}
				} else {
					if (logger.isDebugEnabled()) {
						logger.debug("Accepted taxon (" + mappedAcceptedTaxonId + ") or misapplied name (" + synonymId + " is null");
					}
				}
			} catch (Exception e) {
				logger.error("Error creating synonym relationship: accepted taxon-synonym (" + 
						mappedAcceptedTaxonId + "-" + synonymId + ")", e);
			}
		}
		if (logger.isTraceEnabled()) {
