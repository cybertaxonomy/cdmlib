...
public class PrintPubExportState extends TaxonTreeExportStateBase<PrintPubExportConfigurator, PrintPubExportState> {
    ...
    public Map<PrintPubReferenceSourceType, Integer> getReferenceSourceTypeCounts() {
        ...
        Map<PrintPubReferenceSourceType, Integer> counts = new HashMap<PrintPubReferenceSourceType, Integer>();
    ...
    public PrintPubDocumentRequest createDocumentRequest() {

        PrintPubExportConfigurator config = getConfig();

        return PrintPubDocumentRequest.builder().documentTitle(config.getDocumentTitle()).taxa(getTaxa())
                .featureOrderIndex(getFeatureOrderIndex()).bibliography(getSortedBibliography())
                .includeSynonyms(config.isDoSynonyms())
                .includeSynonymConceptReferences(config.isIncludeSynonymConceptReference())
                .oneLinePerHomotypicGroup(config.isOneLinePerHomotypicGroup())
                .includeScientificNameIndex(config.isGenerateScientificNameIndex())
                .includeCommonNameIndex(config.isGenerateCommonNameIndex())
