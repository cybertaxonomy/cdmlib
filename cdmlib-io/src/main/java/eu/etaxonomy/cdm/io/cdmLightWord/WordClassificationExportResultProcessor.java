...
public class WordClassificationExportResultProcessor {
    ...
    public WordClassificationExportResultProcessor(WordClassificationExportState state) {
        super();
        this.state = state;
        Map<String,String[]> resultMap;
        for (WordClassificationExportTable table: WordClassificationExportTable.values()){
...
//            }
    ...
    public void put(WordClassificationExportTable table, String id, String[] csvLine) {
...
//            }
    ...
    public boolean hasRecord(WordClassificationExportTable table, String id){
        Map<String, String[]> resultMap = result.get(table);
        if (resultMap == null){
            return false;
        }else{
    ...
    public  String[] getRecord(WordClassificationExportTable table, String id){
        return result.get(table).get(id);
    }

    public void put(WordClassificationExportTable table, ICdmBase cdmBase, String[] csvLine) {
       this.put(table, cdmBase.getUuid().toString(), csvLine);
    }

    public void createFinalResult(WordClassificationExportState state) {
        ...
        if (!result.isEmpty() ){
            ...
            for (WordClassificationExportTable table: result.keySet()){
    ...
    private String createCsvLine(WordClassificationExportConfigurator config, String[] csvLine) {
        String lineString = "";
        boolean first = true;
