...
@Component
public class Excel34UImport extends Excel34UImporterBase {
	private static final Logger logger = Logger.getLogger(Excel34UImport.class);
	 ...
	 */
	@Override
    protected void firstPass(Excel34UImportState state) {
...
//		System.out.println("FP:" + state.getCurrentLine());
		Rank rank = null;
		Excel34URow taxonDataHolder = state.getCurrentRow();
	 ...
	 */
	@Override
    protected void secondPass(Excel34UImportState state) {
		...
		try {
			...
			if (cdmUuid != null){
				...
				if (CdmUtils.isNotEmpty(taxonNameStr)) {
					...
					}else if (CdmUtils.isNotEmpty(synonymNameStr)){
						//if (synonymMarkers.contains(nameStatus)){
						//add synonym relationship
						acceptedTaxon = parentTaxon;
						try {
//							Synonym synonym = CdmBase.deproxy(taxonBase,Synonym.class);
							Synonym synonym = CdmBase.deproxy(synonymNameStr,Synonym.class);
	 ...
	 * @param acceptedTaxon
	 */
	private void handleFeatures(Excel34UImportState state, Excel34URow taxonDataHolder, Taxon acceptedTaxon, TaxonNameBase nameUsedInSource) {
		//feature
	...
	private Reference getReferenceAccordingToConfig(Reference value, Excel34UImportState state) {
	...
	private TeamOrPersonBase getAuthorAccordingToConfig(String value, Excel34UImportState state) {
	...
	private Language getFeatureLanguage(String featureLanguage, Excel34UImportState state) {
	 ...
	 * @param parentId
	 */
	private void handleCommonName(Excel34UImportState state, 
	 ...
	 * @return
	 */
	private TaxonBase createTaxon(Excel34UImportState state, Rank rank,
	 ...
	 * @return
	 */
	private TaxonBase<?> createTaxon(Excel34UImportState state, Rank rank, String taxonNameStr, 
	...
	private void makeParent(Excel34UImportState state, Taxon parentTaxon, Taxon childTaxon, Reference citation, String microCitation){
		Reference sec = state.getConfig().getSourceReference();
...
//		Reference sec = parentTaxon.getSec();
	 ...
	 */
	@Override
	protected Excel34URow createDataHolderRow() {
		return new Excel34URow();
	}
	 ...
	 */
	@Override
	protected boolean doCheck(Excel34UImportState state) {
		logger.warn("DoCheck not yet implemented for Excel34UImport");
		return true;
	}
	 ...
	 */
	@Override
	protected boolean isIgnore(Excel34UImportState state) {
		return false;
	}
