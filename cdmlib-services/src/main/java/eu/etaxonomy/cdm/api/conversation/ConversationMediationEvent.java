...
/**
 * The conversation mediation event unifies all events that the ConversationMediator 
 * listens for into one interface.
 * 
 * 
 * @author n.hoffmann
 * @created 24.03.2009
 * @version 1.0
 */
public class ConversationMediationEvent extends AbstractEvent{
	 ...
	 * @param source
	 */
	private ConversationMediationEvent(
			CdmBase entity, 
			Serializable id,
			Object[] state,
			Object[] oldState,
	 ...
	 * @return
	 */
	public static ConversationMediationEvent NewInstance(AbstractEvent event) {

		ConversationMediationEvent mediationEvent = null;
		try{
			if(event instanceof PostInsertEvent){
				PostInsertEvent postEvent = (PostInsertEvent) event;
				mediationEvent = new ConversationMediationEvent(
