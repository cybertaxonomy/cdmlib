...
/**
 * #5833 The new single default cache strategy for {@link Reference references}.
 * As we do have only one {@link Reference} class left which implements multiple
 * interfaces, we may also only need 1 single cache strategy. However, care must
 * be taken as the formatting differs dependent on the type an the in-reference
 * structure.
 *
 * Generally the cache strategy allows to compute 2 formats:<BR>
 * <BR>
 *
 * 1.) for bibliographic references (stored in {@link Reference#getTitleCache()
 * titleCache}).<BR>
 *
 * 2.) for nomenclatural references (stored in
 * {@link Reference#getAbbrevTitleCache() abbrevTitleCache}), but without micro
 * reference (detail).<BR>
 * <BR>
 * The formatting of nomenclatural references with micro references has been
 * moved to {@link NomenclaturalSourceFormatter} and the formatting of short
 * citations has been moved to {@link OriginalSourceFormatter}.
 *
 * @author a.mueller
 * @since 25.05.2016
 */
public class ReferenceDefaultCacheStrategy extends StrategyBase implements IReferenceCacheStrategy {
    ...
    public String getTitleCache(Reference reference) {
        ...
        if (isRealInRef(reference)) {
            ...
            // Article, CdDvd, Generic, Section, Thesis, WebPage (+Book, BookSection but not
            // here)
        ...
        if (reference.getType() == ReferenceType.WebPage && reference.getUri() != null
                && !result.contains(reference.getUri().toString())) {
    ...
    private String addPublishInformation(Reference reference, String result) {
        ...
        } else if (reference != null && isBlank(reference.getPublisher())) {
    ...
    private String getAuthorAndYear(Reference reference, boolean isAbbrev, boolean useFullDatePublished) {
        ...
        String authorStr = (author == null) ? ""
                : CdmUtils.getPreferredNonEmptyString(author.getTitleCache(), author.getNomenclaturalTitleCache(),
                        isAbbrev, trim);
    ...
    private String titleCacheRealInRef(Reference reference, boolean isAbbrev) {
        ...
        // section title
        String title = CdmUtils.getPreferredNonEmptyString(reference.getTitle(), reference.getAbbrevTitle(), isAbbrev,
                trim);
        if (title.matches(".*[.!\\?]")) {
        ...
        if (date == null && inRef != null && inRef.hasDatePublished()) {
            ...
            if (date == null && inRef.isSection() && inRef.getInReference() != null
                    && inRef.getInReference().hasDatePublished()) {
    ...
    private String getInRefAuthorAndTitle(Reference inRef, ReferenceType type, boolean isAbbrev) {
        ...
        if (inRef != null) {
            ...
            String authorStr = (inRefAuthor == null) ? ""
                    : CdmUtils.getPreferredNonEmptyString(inRefAuthor.getTitleCache(),
    ...
    private String titleCacheJournal(Reference reference, boolean isAbbrev) {
        ...
        // title
        result = CdmUtils.getPreferredNonEmptyString(reference.getTitle(), reference.getAbbrevTitle(), isAbbrev, trim);
        ...
        if (team != null) {
            String author = CdmUtils.getPreferredNonEmptyString(team.getTitleCache(), team.getNomenclaturalTitleCache(),
                    isAbbrev, trim);
    ...
    private String titleCacheDefaultReference(Reference reference, boolean isAbbrev) {
        ...
        result = "";
        String titel = CdmUtils.getPreferredNonEmptyString(reference.getTitle(), reference.getAbbrevTitle(), isAbbrev,
                trim);
        if (isNotBlank(titel)) {
            result = titel + blank;
        }
     ...
     * @param reference
     *                                 the reference
     * @param useFullDatePublished
     *                                 weather to add the year only or the full date
    ...
    /**
     * Returns <code>true</code> if the type of the reference originally
     * corresponded to a cache strategy which inherited from
     * {@link InRefDefaultCacheStrategyBase} and in case of type
     * {@link ReferenceType#Generic} if it really has an inreference
     * (reference.getInreference() != null).
     *
     * @param reference
     */
     ...
     * Returns <code>true</code> if the type of the reference originally
     * corresponded to a cache strategy which inherited from
     * {@link NomRefDefaultCacheStrategyBase}.
     *
     * @param type
    ...
    /**
     * Returns year information as originally computed by
