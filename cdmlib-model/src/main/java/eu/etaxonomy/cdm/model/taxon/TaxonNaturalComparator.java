...
//			return 0;
//		}
		if (node1.isAncestor(node2)) return 1;
		if (node2.isAncestor(node1)) return -1;
		if (node1.equals(node2)) return 0;
		
		String[] splitNode1 = node1.treeIndex().split("#");
		String[] splitNode2 = node2.treeIndex().split("#");
		if (splitNode1.length < splitNode2.length) return 1;
		else if (splitNode2.length < splitNode1.length) return -1;
